...
import { addSongs } from '@/redux/song/songSlice';
import { ISong } from '@/types/types';
import { use, useEffect } from 'react';
import { getNewestSongs } from '@/actions/getNewestSongs';
...
const SongCard = dynamic(() => import('@/components/songCard/SongCard'), {
  loading: () => (
    <div className='grid h-full w-full grid-cols-1 gap-4 sm:grid-cols-3 md:grid-cols-5'>
...
const Home = () => {
  ...
  const songs: ISong[] = use(getNewestSongs());
        ...
        <Header>
          <div>
            ...
            <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3'>
              {tiles.map((tile) => (
                <Tiles key={tile.label} image={tile.image} label={tile.label} />
              ))}
            </div>
          </div>
        </Header>
